...
// Workaround for issue with inttypes.h
#ifdef __MINGW32__
...
void TestResults()
{
    ...
    for (int i=0; i<BUFSIZE; ++i)
    {
        ...
        if (saw[i] != expected[i])
        {
            printf("%3d: saw 0x%.8" PRIx32 ", expected 0x%.8" PRIx64 "\n", i, saw[i], expected[i]);
        }
    }
}
#undef BUFSIZE
...
void DoTimingBig(int seed)
{
    ...
    {
        spooky_hash128(buf[i], BUFSIZE, &hash1, &hash2);
    }
    uint64_t z = get_ticks();
    printf("spooky_hash128, uncached: time is %4" PRId64 " milliseconds\n", z-a);
    ...
    z = get_ticks();
    printf("spooky_hash128,   cached: time is %4" PRId64 " milliseconds\n", z-a);
...
void DoTimingSmall(int seed)
{
    ...
    for (int i=1; i <= BUFSIZE; i <<= 1)
    {
        ...
        uint64_t z = get_ticks();
        printf("%d bytes: hash is %.16" PRIx64 " %.16" PRIx64 ", time is %" PRId64 "\n", 
               i, hash1, hash2, z-a);
    }
}
#undef BUFSIZE
        ...
        for (int i=0; i<h*8; ++i)
        {
            ...
            for (int j=0; j<=i; ++j)
            {
                ...
                for (k=0; k<TRIES; ++k)
                {
                    ...
                    for (int l=0; l<h; ++l)
                    {
                        buf1[l] = buf2[l] = (uint8_t)random.Value();
                    }
                    buf1[i/8] ^= (1 << (i%8));
                    if (j != i)
                    {
